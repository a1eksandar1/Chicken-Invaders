...
void UsernameWindow::onOk()
{
      ...
      if (name.compare("") != 0)
      {
            ...
            if (!qry->isValid())
            {
                  ...
                  qry->prepare(
                      "insert into Players (name, score, level, difficulty) values (:name, :score, :level, "
                      ":difficulty)");
                  qry->bindValue(":name", name);
...
void UsernameWindow::onCancel() { delete this; }

UsernameWindow::UsernameWindow(MainWindow *parent)
    : QDialog(parent), ui(new Ui::UsernameWindow), m_ready(false), mw(parent)
{
      ui->setupUi(this);
...
UsernameWindow::~UsernameWindow() { delete ui; }

bool UsernameWindow::ready() const { return m_ready; }

bool UsernameWindow::levelChooseReady() { return m_levelChooseReady; }

void UsernameWindow::setReady(bool value) { m_ready = value; }
