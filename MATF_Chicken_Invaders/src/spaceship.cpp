...
void Spaceship::throw_projectile()
{
    ...
    for(int i=0; i<projectilesLevel; i++) // alociranje projektila
        projectiles.push_back(new Projectile(throwingProjectilesTimer));

    // projectile->setPos(pos().x()+30, pos().y()-40);
    ...
    case 3:
        ...
        break;
    case 4:
        projectiles[0]->setPos(pos().x()-10, pos().y()-25);
        projectiles[1]->setPos(pos().x()+45, pos().y()-40);
        projectiles[2]->setPos(pos().x()+15, pos().y()-40);
        projectiles[3]->setPos(pos().x()+70, pos().y()-25);
        break;
    case 5:
        projectiles[0]->setPos(pos().x()+30, pos().y()-40);
        projectiles[1]->setPos(pos().x()-10, pos().y()-10);
        projectiles[2]->setPos(pos().x()+50, pos().y()-25);
        projectiles[3]->setPos(pos().x()+10, pos().y()-25);
