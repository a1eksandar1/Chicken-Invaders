...
void WaveChicken::advance(int step)
{
      ...
      if (!mw->getFreezeScene())
      {
            if (imgChange == 0)
                  setPixmap(
                      QPixmap(":images/chicken/pinkchicken.png").scaled(width / 13, height / 9, Qt::KeepAspectRatio));
            if (imgChange == 1)
                  setPixmap(
                      QPixmap(":images/chicken/pinkchicken2.png").scaled(width / 13, height / 9, Qt::KeepAspectRatio));
            ...
            if (!wave and pos().y() < 10) orientation = -orientation;

            if (pos().y() > height or pos().x() > width) die();
      }
}
