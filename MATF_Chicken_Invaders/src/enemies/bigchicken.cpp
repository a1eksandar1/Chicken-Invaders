...
BigChicken::BigChicken(MainWindow *parent, QTimer *cleanTimer, int shotCounter) :
    mw(parent), cleanTimer(cleanTimer)
{
    if(mw->isHard())
...
void BigChicken::die()
{
    ...
    else
    {
        if(!dead)
        {
            ...
            setPixmap(QPixmap(":images/chicken/shot_chicken.png").scaled(height/2, height/2, Qt::KeepAspectRatio));
            imgChange = 3;
