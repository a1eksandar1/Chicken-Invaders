...
{
      shotCounter--;
      if (balloonCounter > 0) balloonCounter--;

      if (balloonCounter == 2)
            setPixmap(
                QPixmap(":images/chicken/balloonChicken2.png").scaled(width / 13, height / 9, Qt::KeepAspectRatio));
      if (balloonCounter == 1)
            setPixmap(
                QPixmap(":images/chicken/balloonChicken1.png").scaled(width / 13, height / 9, Qt::KeepAspectRatio));
...
void BalloonChicken::advance(int step)
{
      ...
      if (!mw->getFreezeScene())
      {
            ...
            if (pos().x() + width / 13 * (num1 - 1 - m) > width - width / 13) orientation = -10;

            if (pos().x() - width / 13 * (m) < 0) orientation = 10;
